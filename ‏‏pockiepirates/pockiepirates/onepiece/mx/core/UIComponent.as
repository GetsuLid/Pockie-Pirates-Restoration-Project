   ...
   public class UIComponent extends FlexSprite implements IAutomationObject, IChildList, IDeferredInstantiationUIComponent, IFlexDisplayObject, IFlexModule, IInvalidating, ILayoutManagerClient, IPropertyChangeNotifier, IRepeaterClient, ISimpleStyleClient, IStyleClient, IToolTipManagerClient, IUIComponent, IValidatorListener, IStateClient, IConstraintClient
   {
      ...
      public function invalidateDisplayList() : void
      {
         if(!invalidateDisplayListFlag)
         {
            ...
            if(isOnDisplayList() && Boolean(UIComponentGlobals.layoutManager))
            {
      ...
      mx_internal function initThemeColor() : Boolean
      {
         ...
         if((tc === null || !StyleManager.isValidStyleValue(tc)) && (Boolean(styleName) && Boolean(!(styleName is ISimpleStyleClient))))
         {
      ...
      public function validationResultHandler(event:ValidationResultEvent) : void
      {
         ...
         {
            if(validationSubField != null && validationSubField != "" && Boolean(event.results))
            {
            ...
            if(Boolean(msg) && errorString != msg)
            {
               errorString = msg;
      ...
      override public function addChild(child:DisplayObject) : DisplayObject
      {
         ...
         var index:int = Boolean(overlayReferenceCount) && child != overlay ? int(Math.max(0,super.numChildren - 1)) : super.numChildren;
         addingChild(child);
      ...
      {
         var sm:ISystemManager = systemManager;
         if(Boolean(sm) && (Boolean(sm.stage) || Boolean(sm.useSWFBridge())))
         {
      ...
      public function deleteReferenceOnParentDocument(parentDocument:IFlexDisplayObject) : void
      {
         ...
         if(Boolean(id) && id != "")
         {
            ...
            else
            {
               ...
               for(j = int(stack.length - 1); j > 0; j--)
               {
                  if(stack[j].length == 0)
                  {
      ...
      public function setStyle(styleProp:String, newValue:*) : void
      {
         ...
         var isProtoChainInitialized:Boolean = inheritingStyles != UIComponent.STYLE_UNINITIALIZED;
      ...
      {
         if(Boolean(parent && parent.instanceIndices) && (Boolean(!parent.isDocument || parent != descriptor.document)) && !_instanceIndices)
         {
      ...
      override public function addChildAt(child:DisplayObject, index:int) : DisplayObject
      {
         ...
         if(Boolean(overlayReferenceCount) && child != overlay)
         {
      ...
      private function callLaterDispatcher2(event:Event) : void
      {
         ...
         var sm:ISystemManager = systemManager;
         if(Boolean(sm) && (Boolean(sm.stage || sm.useSWFBridge())) && listeningForRender)
         {
      ...
      {
         var len:int = int(_effectsStarted.length);
