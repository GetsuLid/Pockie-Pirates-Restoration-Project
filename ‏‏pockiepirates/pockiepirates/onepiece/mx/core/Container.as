package mx.core
{
   ...
   public class Container extends UIComponent implements IContainer, IDataRenderer, IFocusManagerContainer, IListItemRenderer, IRawChildrenContainer
   {
      ...
      protected function scrollChildren() : void
      {
         ...
         if(Boolean(border) && Boolean(border is IRectangularBorder) && IRectangularBorder(border).hasBackgroundImage)
         {
      ...
      override public function notifyStyleChangeInChildren(styleProp:String, recursive:Boolean) : void
      {
         ...
         {
            if(Boolean(contentPane) || Boolean(i < _firstChildIndex) || i >= _firstChildIndex + _numChildren)
            {
      ...
      private function createOrDestroyScrollbars(needHorizontal:Boolean, needVertical:Boolean, needContentPane:Boolean) : Boolean
      {
         ...
         if(needHorizontal)
         {
            if(!horizontalScrollBar)
            {
               ...
               if(Boolean(horizontalScrollBarStyleName) && horizontalScrollBar is ISimpleStyleClient)
               {
         ...
         if(needVertical)
         {
            if(!verticalScrollBar)
            {
               ...
               if(Boolean(verticalScrollBarStyleName) && verticalScrollBar is ISimpleStyleClient)
               {
      ...
      override public function addChildAt(child:DisplayObject, index:int) : DisplayObject
      {
         ...
         if(Boolean(formerParent) && !(formerParent is Loader))
         {
            ...
            {
               index = index == numChildren ? int(index - 1) : index;
            }
      ...
      override public function set focusPane(o:Sprite) : void
      {
         ...
         if(Boolean(o) && Boolean(contentPane))
         {
            o.x = contentPane.x;
            o.y = contentPane.y;
      ...
      override public function removeChild(child:DisplayObject) : DisplayObject
      {
         ...
         if(child is IDeferredInstantiationUIComponent && Boolean(IDeferredInstantiationUIComponent(child).descriptor))
         {
            ...
            {
               n = int(createdComponents.length);
               for(i = 0; i < n; i++)
               {
      ...
      override public function getChildIndex(child:DisplayObject) : int
      {
         ...
         return int(super.getChildIndex(child) - _firstChildIndex);
      }
      ...
      {
         if(Boolean(_viewMetricsAndPadding) && (Boolean(!horizontalScrollBar || horizontalScrollPolicy == ScrollPolicy.ON)) && (!verticalScrollBar || verticalScrollPolicy == ScrollPolicy.ON))
         {
      ...
      private function createScrollbarsIfNeeded(bounds:Rectangle) : Boolean
      {
         ...
         var needHorizontal:Boolean = horizontalScrollPolicy == ScrollPolicy.ON;
         var needVertical:Boolean = verticalScrollPolicy == ScrollPolicy.ON;
         ...
         if(Boolean(needHorizontal && needVertical && horizontalScrollPolicy == ScrollPolicy.AUTO && verticalScrollPolicy == ScrollPolicy.AUTO && horizontalScrollBar && verticalScrollBar) && Boolean(newViewableWidth + verticalScrollBar.minWidth >= newScrollableWidth) && newViewableHeight + horizontalScrollBar.minHeight >= newScrollableHeight)
         {
            needHorizontal = needVertical = false;
         }
         else if(Boolean(needHorizontal && !needVertical && verticalScrollBar) && Boolean(horizontalScrollPolicy == ScrollPolicy.AUTO) && newViewableWidth + verticalScrollBar.minWidth >= newScrollableWidth)
         {
      ...
      override public function initialize() : void
      {
         var props:* = undefined;
         var message:String = null;
         if(Boolean(isDocument) && Boolean(documentDescriptor) && !processedDescriptors)
         {
      ...
      override mx_internal function childRemoved(child:DisplayObject) : void
      {
         ...
         if(Boolean(contentPane) && !autoLayout)
         {
            forceLayout = true;
            UIComponentGlobals.layoutManager.addEventListener(FlexEvent.UPDATE_COMPLETE,layoutCompleteHandler,false,0,true);
         }
      ...
      {
         var rect:Rectangle = Boolean(contentPane) && Boolean(contentPane.scrollRect) ? contentPane.scrollRect : null;
         if(rect)
         {
            contentPane.scrollRect = null;
         }
      ...
      override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number) : void
      {
         ...
         if(Boolean(contentPane) && Boolean(contentPane.scrollRect))
         {
            ...
            if(!_clipContent || isNaN(Number(backgroundColor)) || backgroundColor === "" || !(Boolean(horizontalScrollBar) || Boolean(verticalScrollBar)) && !cacheAsBitmap)
            {
