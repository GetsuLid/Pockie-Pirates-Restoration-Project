package mx.managers
{
   ...
   public class FocusManager implements IFocusManager
   {
      ...
      private function tabEnabledChangeHandler(event:Event) : void
      {
         ...
         var n:int = int(focusableObjects.length);
      ...
      private function getNextFocusManagerComponent2(backward:Boolean = false, fromObject:DisplayObject = null, fromIndex:int = -2) : FocusInfo
      {
         ...
         if(i == -1)
         {
            if(backward)
            {
               i = int(focusableCandidates.length);
            }
            bSearchAll = true;
         }
      ...
      private function getIndexOfFocusedObject(o:DisplayObject) : int
      {
         ...
         var n:int = int(focusableCandidates.length);
         var i:int = 0;
         for(i = 0; i < n; i++)
         {
      ...
      private function removeFromParentBridge(event:Event) : void
      {
         ...
         if(sm.useSWFBridge())
         {
            ...
            if(Boolean(bridge) && !(form.systemManager is SystemManagerProxy))
            {
      ...
      private function setFocusToComponent(o:Object, shiftKey:Boolean) : void
      {
         ...
         if(o)
         {
            if(o is ISWFLoader && Boolean(ISWFLoader(o).swfBridge))
            {
      ...
      private function keyDownHandler(event:KeyboardEvent) : void
      {
         ...
         if(Boolean(defaultButtonEnabled && event.keyCode == Keyboard.ENTER) && Boolean(defaultButton) && Boolean(defButton.enabled))
         {
      ...
      private function getMaxTabIndex() : int
      {
         ...
         for(var i:int = 0; i < n; i++)
         {
            t = Number(focusableObjects[i].tabIndex);
            if(!isNaN(t))
            {
               z = Math.max(z,t);
            }
      ...
      private function sortFocusableObjectsTabIndex() : void
      {
         ...
         for(var i:int = 0; i < n; i++)
         {
            ...
            if(Boolean(c && c.tabIndex) && Boolean(!isNaN(Number(c.tabIndex))) || focusableObjects[i] is ISWFLoader)
            {
      ...
      private function dispatchEventFromSWFBridges(event:Event, skip:IEventDispatcher = null) : void
      {
         ...
         if(!popup)
         {
            ...
            if(Boolean(parentBridge) && parentBridge != skip)
            {
               clone = event.clone();
      ...
      public function set showFocusIndicator(value:Boolean) : void
      {
         ...
         if(changed && !popup && Boolean(form.systemManager.swfBridgeGroup))
         {
