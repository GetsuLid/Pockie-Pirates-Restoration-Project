package mx.managers
{
   ...
   public class SystemManager extends MovieClip implements IChildList, IFlexDisplayObject, IFlexModuleFactory, ISystemManager, ISWFBridgeProvider
   {
      ...
      private static function areRemotePopUpsEqual(form1:Object, form2:Object) : Boolean
      {
         ...
         if(Boolean(remotePopUp1.window == remotePopUp2.window) && Boolean(remotePopUp1.bridge) && Boolean(remotePopUp2.bridge))
         {
            return true;
         }
         return false;
      ...
      {
         var newListener:StageEventProxy = null;
      ...
      private function mouseDownHandler(event:MouseEvent) : void
      {
         ...
         if(Boolean(bridge) && bridgeToFocusManager[bridge] == document.focusManager)
         {
            if(isTopLevelRoot())
            {
               activateForm(document);
         ...
         if(numModalWindows == 0)
         {
            if(!isTopLevelRoot() || forms.length > 1)
            {
               ...
               isApplication = Boolean(document.rawChildren.contains(p));
               while(p)
               {
                  for(i = 0; i < n; i++)
                  {
      ...
      {
         var sbRoot:DisplayObject = null;
         var sendToSbRoot:Boolean = false;
         var bridgeEvent:SWFBridgeEvent = null;
      ...
      mx_internal function initialize() : void
      {
         ...
         {
            usePreloader = Boolean(info()["usePreloader"]);
         }
      ...
      private function initializeTopLevelWindow(event:Event) : void
      {
         ...
         else if(topLevel && Boolean(stage))
         {
            sandboxRoot = getSandboxRoot();
            if(sandboxRoot != this)
            {
