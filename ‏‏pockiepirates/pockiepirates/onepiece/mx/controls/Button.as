package mx.controls
{
   ...
   public class Button extends UIComponent implements IDataRenderer, IDropInListItemRenderer, IFocusManagerComponent, IListItemRenderer, IFontContextComponent, IButton
   {
      ...
      private function previousVersion_measure() : void
      {
         ...
         tempCurrentIcon = getCurrentIcon();
         ...
         if(Boolean(currentSkin) && (isNaN(skinMeasuredWidth) || isNaN(skinMeasuredHeight)))
         {
      ...
      mx_internal function viewIconForPhase(tempIconName:String) : IFlexDisplayObject
      {
         ...
         if(!newIconClass)
         {
            ...
            if(!checkedDefaultIcon && Boolean(newIconClass))
            {
      ...
      mx_internal function viewSkinForPhase(tempSkinName:String, stateName:String) : void
      {
         ...
         if(!newSkinClass)
         {
            ...
            if(!checkedDefaultSkin && Boolean(newSkinClass))
            {
      ...
      public function set data(value:Object) : void
      {
         ...
         _data = value;
         if(Boolean(_listData) && Boolean(_listData is DataGridListData) && DataGridListData(_listData).dataField != null)
         {
      ...
      mx_internal function layoutContents(unscaledWidth:Number, unscaledHeight:Number, offset:Boolean) : void
      {
         ...
         var bm:EdgeMetrics = Boolean(currentSkin) && Boolean(currentSkin is IBorder) && !(currentSkin is IFlexAsset) ? IBorder(currentSkin).borderMetrics : null;
         if(bm)
         {
      ...
      override protected function commitProperties() : void
      {
         ...
         if(enabledChanged)
         {
            ...
            if(Boolean(currentIcon) && currentIcon is IUIComponent)
            {
      ...
      private function previousVersion_layoutContents(unscaledWidth:Number, unscaledHeight:Number, offset:Boolean) : void
      {
         ...
         var n:Number = offset ? buttonOffset : 0;
         var textAlign:String = getStyle("textAlign");
         var bm:EdgeMetrics = Boolean(currentSkin) && currentSkin is IRectangularBorder ? IRectangularBorder(currentSkin).borderMetrics : null;
         var viewWidth:Number = unscaledWidth;
         ...
         if(labelPlacement == ButtonLabelPlacement.LEFT || labelPlacement == ButtonLabelPlacement.RIGHT)
         {
            ...
            if(labelPlacement == ButtonLabelPlacement.RIGHT)
            {
               ...
               if(centerContent)
               {
                  ...
                  {
                     labelX += viewWidth - labelWidth - iconWidth - horizontalGap - paddingLeft;
                  }
                  else
                  {
            ...
            {
               labelY += Math.max(0,(viewHeight - labelHeight) / 2) + paddingTop;
               iconY += Math.max(0,(viewHeight - iconHeight) / 2 - 1) + paddingTop;
            }
         }
         else
         {
            ...
            else
            {
               ...
               if(centerContent)
               {
                  labelY += (viewHeight - labelHeight - iconHeight - verticalGap) / 2 + 1;
               }
