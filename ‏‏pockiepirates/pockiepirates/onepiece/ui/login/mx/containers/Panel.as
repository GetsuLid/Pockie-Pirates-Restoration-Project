package mx.containers
{
   ...
   public class Panel extends Container implements IConstraintLayout, IFontContextComponent
   {
      ...
      {
         if(Boolean(controlBar) && child == controlBar)
         {
            return numChildren;
         }
      ...
      override protected function createChildren() : void
      {
         ...
         if(!closeButton)
         {
            ...
            closeButton.mx_internal::upSkinName = "closeButtonUpSkin";
            closeButton.mx_internal::overSkinName = "closeButtonOverSkin";
            closeButton.mx_internal::downSkinName = "closeButtonDownSkin";
            closeButton.mx_internal::disabledSkinName = "closeButtonDisabledSkin";
            closeButton.mx_internal::skinName = "closeButtonSkin";
      ...
      override public function set cacheAsBitmap(value:Boolean) : void
      {
         ...
         if(cacheAsBitmap && !mx_internal::contentPane && cachePolicy != UIComponentCachePolicy.OFF && Boolean(getStyle("backgroundColor")))
         {
      ...
      override public function createComponentsFromDescriptors(recurse:Boolean = true) : void
      {
         ...
         if(lastChild is ControlBar)
         {
            ...
            {
               mx_internal::contentPane.removeChild(DisplayObject(lastChild));
            }
            else
            {
      ...
      override protected function layoutChrome(unscaledWidth:Number, unscaledHeight:Number) : void
      {
         ...
         if(getQualifiedClassName(mx_internal::border) == "mx.skins.halo::PanelSkin" && getStyle("borderStyle") != "default" && Boolean(bt))
         {
         ...
         var x:Number = Number(bm.left);
         var y:Number = Number(bm.top);
         ...
         if(headerHeight > 0 && height >= headerHeight)
         {
            ...
            if(titleIconObject)
            {
               h = Number(titleIconObject.height);
            ...
            {
               h = Number(titleTextField.nonZeroTextHeight);
            }
            else
            {
               h = Number(titleTextField.getUITextFormat().measureText(titleTextField.text).height);
            }
            offset = (headerHeight - h) / 2;
            borderWidth = bm.left + bm.right;
            titleTextField.move(leftOffset,offset - 1);
            titleTextField.setActualSize(Math.max(0,unscaledWidth - leftOffset - rightOffset - borderWidth),h + UITextField.mx_internal::TEXT_HEIGHT_PADDING);
            if(FlexVersion.compatibilityVersion < FlexVersion.VERSION_3_0)
            {
               h = Number(statusTextField.textHeight);
            }
            else
            {
               h = statusTextField.text != "" ? Number(statusTextField.getUITextFormat().measureText(statusTextField.text).height) : 0;
            }
            offset = (headerHeight - h) / 2;
         ...
         if(controlBar)
         {
            cx = Number(controlBar.x);
            cy = Number(controlBar.y);
            cw = Number(controlBar.width);
            ch = Number(controlBar.height);
      ...
      override public function get viewMetrics() : EdgeMetrics
      {
         ...
         if(FlexVersion.compatibilityVersion < FlexVersion.VERSION_3_0)
         {
            ...
            vm = panelViewMetrics;
            o = super.viewMetrics;
            bt = Number(getStyle("borderThickness"));
            btl = Number(getStyle("borderThicknessLeft"));
            btt = Number(getStyle("borderThicknessTop"));
            btr = Number(getStyle("borderThicknessRight"));
            btb = Number(getStyle("borderThicknessBottom"));
      ...
      private function setControlBar(newControlBar:IUIComponent) : void
      {
         ...
         if(Boolean(controlBarStyleName) && controlBar is ISimpleStyleClient)
         {
      ...
      private function titleBar_mouseDownHandler(event:MouseEvent) : void
      {
         ...
         if(enabled && Boolean(isPopUp) && isNaN(regX))
         {
            startDragging(event);
         }
      }
      ...
      {
         var headerHeight:Number = Number(getStyle("headerHeight"));
      ...
      private function showTitleBar(show:Boolean) : void
      {
         ...
         var n:int = int(titleBar.numChildren);
      ...
      override protected function commitProperties() : void
      {
         ...
         if(hasFontContextChanged())
         {
            if(titleTextField)
            {
               childIndex = int(titleBar.getChildIndex(DisplayObject(titleTextField)));
            ...
            if(statusTextField)
            {
               childIndex = int(titleBar.getChildIndex(DisplayObject(statusTextField)));
